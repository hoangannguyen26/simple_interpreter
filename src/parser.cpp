...
Parser::Parser(const LexerPtr &lexer) : m_exitFromBlock(false),
                                        m_lexer(lexer),
...
ASTPtr Parser::expr()
{
    ...
    */
    ASTPtr result = term();
    while (m_currentToken->m_type == TokenType::PLUS || m_currentToken->m_type == TokenType::MINUS || m_currentToken->m_type == TokenType::GREAT || m_currentToken->m_type == TokenType::LESS)
    {
        TokenPtr token = m_currentToken;
        if (token->m_type == TokenType::PLUS)
        {
